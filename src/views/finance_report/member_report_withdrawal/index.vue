...
import { ref } from "vue";
import { useMemberReportWithdrawal } from "./utils/hook";
...
defineOptions({ name: "FinanceReportMemberReportWithdrawal" });
...
<template>
  <div class="main">
    ...
    >
      <el-form-item :label="$t('finance_report.withdrawalDate')">
        <el-date-picker
          v-model="dateRange"
          type="daterange"
      ...
      >
        <el-select v-model="searchForm.paymentGroup" class="!w-[160px]">
          <el-option
            v-for="item in paymentGroupOptions"
            :key="item.value"
            :label="item.label"
            :value="item.value"
          />
    ...
    >
      <template #buttons>
        <span v-if="updatedAt" class="mr-4 text-[var(--el-text-color-secondary)]">
